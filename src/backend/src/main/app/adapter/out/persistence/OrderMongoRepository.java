...
public interface OrderMongoRepository extends ReactiveMongoRepository<OrderDocument, String> {
    ...
    Flux<OrderDocument> findByCreatedAtBetween(ZonedDateTime startDate, ZonedDateTime endDate);
    
    /**
     * 사용자별 특정 기간 내 주문 조회
     */
    @Query("{ 'user.id': ?0, 'createdAt': { $gte: ?1, $lte: ?2 } }")
    Flux<OrderDocument> findByUserIdAndCreatedAtBetween(String userId, ZonedDateTime startDate, ZonedDateTime endDate);
    ...
    Mono<Long> countByCreatedAtBetween(ZonedDateTime startDate, ZonedDateTime endDate);
