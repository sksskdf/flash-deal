...
```javascript
{
  validator: {
    $jsonSchema: {
      bsonType: "object",
      required: ["title", "category", "price", "schedule", "status"],
      properties: {
        dealType: { enum: ["flash", "time", "group"], description: "딜 타입 (선택)" },
...
```javascript
{
  ...
  items: [
    {
      ...
      snapshot: {
        title: "무선 노이즈 캔슬링 헤드폰",
        // (현재 구현) subtitle, category는 생략될 수 있음
  ...
  // === 멱등성 키 ===
  idempotencyKey: "idem-user123-1697184600-abc",  // 중복 주문 방지 (unique, sparse 권장)
...
// (주의) 인덱스 적용 전 다중 문서가 존재할 수 있으며, 서비스는 첫 건을 반환하도록 방어적으로 동작함
